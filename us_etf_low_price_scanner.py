...
def check_alert_conditions(hist_data, now_prices):
    ...
    1. comparing current price with past moving-50/25 percentile price list,
       if the price is 5% lower than historical percentile prices, cur_alert_cnt++
    2. threshold for cur_alert_cnt = 4
    """

    ma_info = get_N_MA(hist_data)
    DOWN_PERCENT_THRESHOLD = 5
    alert_msgs = {}
...
if __name__ == '__main__':
    ...
    start = time.time()
    rtn_alert_msgs = {}
    split_tasks = chunk_list(ALL_TICKERS, 10)
    futures = []
    ...
    with open('{}{}us_etf_rtn_alert_msgs.json'.format(WORKING_FOLDER_PATH, SEP), 'w') as fp:
        json.dump(rtn_alert_msgs, fp)

    with open('{}{}us_top_etf_low_price_output.txt'.format(WORKING_FOLDER_PATH, SEP), 'w') as fp:
